-- ============================================================================
-- 003_fix_producer_registration_rpc.sql
-- Module: VyaparSetu Producer Authentication RPC Fix (Step 3L)
-- Description: Fixes PostgreSQL error 42883 by replacing pseudo-qualified
--              pg_catalog.coalesce() calls with native SQL COALESCE construct,
--              while preserving strict security hardening (SECURITY DEFINER,
--              empty search_path, role boundaries, and validation).
-- ============================================================================
...
BEGIN
    ...
    -- 3. Normalize input name using native SQL COALESCE construct
    v_normalized_name := pg_catalog.trim(COALESCE(p_full_name, ''));
